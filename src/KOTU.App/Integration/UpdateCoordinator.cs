...
/// <b>① 상시 2분 간격 자동 확인</b>(프로세스당 타이머 1개 — 창 수와 무관) ·
...
///
/// 정책 이력 — 같은 주제로 <b>다섯 번</b> 뒤집혔다. 또 뒤집기 전에 읽을 것:
...
///  · v0.148.0 (A125) <b>현행</b> — 수동 "Check now" 버튼을 뺐다. A114의 ①② 자동 경로는 그대로라
///    확인 빈도는 바뀌지 않는다. 사람이 확인을 시키는 손잡이만 사라진 것이다(설정 화면을 열면
...
internal static class UpdateCoordinator
{
    ...
    /// 2분 주기 타이머가 전부 여기로 모인다(수동 버튼 경로는 A125/v0.148.0에서 사라졌다).
    /// 진행 중이면 곧바로 반환한다 —
