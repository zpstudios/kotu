    ...
    IUntitledContentSource, IPrintPageProvider
{
    ...
    private void ApplyLoadedText(string path, LoadedText loaded)
    {
        ...
        ShowTextFitState();
        _shownPath = path;
        // A247: New 버튼은 상시 활성 — 종전의 콘텐츠 열림 비활성(UpdateNewFileButton)은 제거됐다.
    ...
    /// <summary>
    /// A247: 셸→뷰 무제 개시 진입로(IUntitledContentSource) — WindowManager의 새 창 경로가
    /// 문서 모듈을 띄운 직후 부른다. 미저장 가드는 걸지 않으므로 호출자 계약 = 빈 상태 뷰
    /// (새 창)에서만 부를 것(StartUntitled 자체는 콘텐츠 위에서도 안전하지만 파기가 된다).
    /// </summary>
    public void OpenUntitled() => StartUntitled();
    ...
    private void HidePdf()
    {
        ...
        ShowTextFitState(); // A145→A214: 텍스트 갈래 상태로 — 파일·무제가 있으면 활성,
                            // 빈 화면 복귀면 비활성 "1:1" 상자(판정은 저쪽이 _path·_untitled로 한다)
    ...
    private void ShowTextFitState()
    {
        ...
        FitButton.Content = BuildOriginalRatioBox();
    ...
    /// 조립은 <see cref="BuildOriginalRatioBox"/> 한 곳 — 위 빈 화면 갈래도 같은 것을 쓴다.
    ...
    /// (v0.174.1 실사례의 일반형. 이 모듈은 호출 지점이 둘이라 특히 중요하다 —
    /// <see cref="ShowTextFitState"/>의 빈 화면 갈래와 <see cref="UpdateFitButton"/>).
    /// 세 모듈이 각자 같은 메서드를 가진다(모듈 간 공유 금지).
