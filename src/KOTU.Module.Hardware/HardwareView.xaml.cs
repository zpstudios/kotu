...
/// 호스트한다. F11/F12·2연타·경계 버튼·힌트는 파일 모듈과 같은 셸 상태 머신 공통이고,
/// 진입 기본 = 좌·우 사이드바 모두(A109 → A273 좌만 → A314/2026-09-02에서 좌·우 열림으로 재개정).
/// 열 수 = 셸 도크 수 신호(ISidebarAwareView — A119 개정):
    ...
    ITrayStatusProvider, ISidebarAwareView, ISidePanelProvider
{
    ...
    ///   (A151에서 82/82 → A186이 우측 82→44 → A236이 하드웨어 화면 좌측 82→44 → A305 배치 2가
    ///   셸 모드 버튼을 2개로 되돌려 우측 44→82 → A312가 버튼 1개 3상태로 재통합해 우측 82→44:
    ///   지금 이 모듈에서는 44/44다. 값은 셸 MainWindow.UpdateOpenFileButton이 정한다).
    ///   여백이 바뀌어도 이 상수는 **불변**이다 — 고정 요소 합은 BarGrid 안의 칸들만 세기 때문.
    ///   바뀌는 것은 "창 폭 얼마에서 임계에 닿는가"뿐이고, 그 파생 사실은 아래
    ///   <see cref="UpdateBarDensity"/> 주석이 기록한다.
    ...
    ///   26 감소로 **574**.)
    /// ※ "최소 창에서의 결과"는 셸 여백이 바뀔 때마다 다시 검산한다(상수가 아니라 파생 사실).
    ///   A293 시점: 여백 합 88(좌 44 + 우 44) → 최소 창 720 기준 BarGrid 약 632, 임계 574에
    ///   여유 58 — 임계 600이던 A259 시점부터 이미 맥박이 내려가지 않는 상태였다.
    ///   A305 배치 2 재검산: 셸 모드 버튼이 2개가 되어 우측 여백 44→82 → 여백 합 126 →
    ///   최소 창 720 기준 BarGrid 약 594, 임계 574에 여유 20이었다.
    ///   **A312 재검산**: 버튼이 1개 3상태로 재통합되어 우측 여백 82→44 → 여백 합 <b>88</b>
    ///   (좌 44 + 우 44 — A293 시점 값으로의 복귀) → 최소 창 720 기준 BarGrid 약 <b>632</b>,
    ///   임계 574에 여유 <b>58</b>. 맥박은 계속 내려가지 않으므로 판정식·상수 모두 무변경이고,
    ///   달라진 것은 여유 폭(20 → 58 회복)뿐이다.
    ///   (더 앞선 이력: "최소 창 720에서 BarGrid 약 556"은 여백이 82/82이던 A151 시절 값.)
    ...
    /// 초기값 8 = 도크 0(전폭) 기준. 정보 모듈 진입 기본(A109 → A273 좌만 → A314: 좌·우 모두)은
    /// 뷰 교체 직후 셸의 첫 푸시가 4열로 잡는다 — 초기값이 화면에 남는 구간은 없다.</summary>
    private int _centerColumns = 8;

    /// <summary>
    /// 셸 푸시(A60 3차 신설 → A119 개정 계약): 공간을 차지 중인 사이드바(불투명 도크) 수(0/1/2)를
    /// 받아 열 수로 환산한다 — A168(v0.165.0)에서 **8/6/4**(구 4/3/2). A176: 반투명 축 폐지로
    /// "열림 = 도크"라 예외 셈이 없다. 호출원은 셸 ApplyOverlayStates(사이드바
    /// 상태 변경의 단일 종착점 — F11/F12 단타·경계 핀 버튼·모듈 진입 기본(A109)이 전부 그리로
