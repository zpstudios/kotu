    ...
    ITrayStatusProvider, ISidebarAwareView, ISidePanelProvider
{
    ...
    ///   셸 모드 버튼을 2개로 되돌려 우측 44→82: 지금 이 모듈에서는 44/82다.
    ///   값은 셸 MainWindow.UpdateOpenFileButton이 정한다).
    ...
    ///   최소 창 720 기준 BarGrid 약 <b>594</b>, 임계 574에 여유 <b>20</b>. 여전히 맥박은
    ///   내려가지 않으므로 판정식·상수 모두 무변경이고, 달라진 것은 여유 폭뿐이다.
    ///   (더 앞선 이력: "최소 창 720에서 BarGrid 약 556"은 여백이 82/82이던 A151 시절 값.)
    ///   실기기 확인 포인트: 최소 창에서 맥박이 보이는지(여유가 58 → 20으로 줄었다).
