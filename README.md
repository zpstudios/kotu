...
파일을 더블클릭하면 확장자에 맞는 기능이 하나의 창에서 열립니다. 압축 파일이면 내부 탐색, 이미지면 뷰어, 동영상이면 플레이어. 각 기능은 검증된 엔진(7-Zip, libvlc 등)을 통합하고, KOTU는 하나로 묶인 경험에 집중합니다.
...
| 탐색기 통합 | ✅ v0.8.0 | 파일 연결 등록(모듈별, 현재 사용자 범위) / 우클릭 "여기에 풀기"·"KOTU-archive로 압축" / 설정에서 켜고 끔 |
...
- v0.13.0 — 내장 테스트 클립(색감 영상+스피커 음악, 빈 상태 ▶) + 파형 수정(인스턴스 옵션, 음악/동영상 인스턴스 분리)
- v0.12.0 — 음악 재생 파형 표시(libvlc visual) + 스레딩 세분화(이미지 폴더 스캔·자막 탐지·자막 변환 백그라운드화)
- v0.11.0 — 플레이어에 음악 파일 재생 추가 + 첫 실행 멎음 수정(libvlc 초기화 백그라운드화)
...
KOTU 자체 코드는 [MIT](LICENSE)입니다. 함께 배포·사용되는 외부 구성요소는 각자의 라이선스를 따릅니다 — [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) 참고 (7-Zip: LGPL, libvlc: LGPL, LibreHardwareMonitor: MPL-2.0 등).
