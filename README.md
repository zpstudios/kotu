...
- **`WinUtil-win-Setup.exe` (권장)** — 설치 한 번이면 이후 새 버전이 나올 때 앱이 알아서 업데이트합니다. 현재 사용자 범위 설치라 관리자 권한이 필요 없습니다.
- `WinUtil-*-win-x64.zip` — 설치 없는 수동 포터블 (자동 업데이트 없음).

어느 쪽이든 압축(7z.dll)·동영상(libvlc) 엔진이 전부 동봉되어 바로 동작합니다.
...
- v0.9.0 — Setup.exe 설치본·자동 업데이트 (Velopack)
