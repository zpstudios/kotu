...
별도 `test-results-...` 아티팩트로 보관한다. 공식 Release는 만들지 않는다.
...
자동 업데이트 피드는 기존 공식 GitHub Releases를 유지한다. 새 브랜치 빌드는 자동 업데이트되지
않으므로 다음 시험본은 해당 커밋의 설치 EXE를 다시 다운로드해야 한다. 더 높은 정식 버전으로
업데이트하면 브랜치 코드가 정식 코드로 교체될 수 있다.
...
아래는 1단계 완료 당시의 계획이다. 현재 완료·검증 상태는
