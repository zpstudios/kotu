...
| F2 이름 변경 | 지원(인라인 편집) | ✅ 인라인 편집 — **2차 v0.125.0** | | 다중 선택이어도 **첫 항목(SelectedItem)만**. 파일은 확장자 제외 부분 선택(탐색기 관례). Enter/포커스 상실 = 커밋, Esc = 취소. 충돌·빈 이름·잘못된 문자 = **커밋 안 함 + 안내 + 원복**(자동 "(2)" 없음 — 사용자 의도와 다른 결과 방지). 우클릭 메뉴 Rename도 같은 편집 진입. **A158: 셸 좌/우 패널 키가 F1/F2에서 F11/F12로 옮겨가 F2 겹침이 소멸** — 이름변경은 F2 그대로이고(사용자 확정), "선택 항목이 있을 때만 소비"라는 A118 시절 규칙도 무변경이다 |
