...
public sealed class ResumeCompatibilityTests : IDisposable
{
    ...
    public void Legacy_json_reads_and_writes_without_changing_other_module()
    {
        ...
        Assert.Equal(new DateTimeOffset(2026, 2, 3, 4, 5, 6, TimeSpan.FromHours(9)), original[0].UpdatedAt);
        Assert.Equal(90000L, store.GetResumePositionMs(@"C:\movie.mp4"));
        store.Report(@"C:\movie.mp4", 120_000, 600_000);
        ...
        Assert.Equal(60000L, json.RootElement.GetProperty("audio.resume")[0].GetProperty("PositionMs").GetInt64());
        var reloaded = new PlaybackResumeStore(new JsonSettingsService(_path));
        Assert.Equal(120_000, reloaded.GetResumePositionMs(@"C:\movie.mp4"));
    }
